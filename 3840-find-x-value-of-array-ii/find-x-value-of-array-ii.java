class Solution {
    class Node {
        int totalProd;
        int[] prefCount;
        
        Node(int k) {
            prefCount = new int[k];
        }
    }
    
    private Node[] tree;
    private int K;
    
    private Node merge(Node left, Node right) {
        if (left == null) return right;
        if (right == null) return left;
        
        Node res = new Node(K);
        res.totalProd = (left.totalProd * right.totalProd) % K;
        
        for (int i = 0; i < K; i++) {
            res.prefCount[i] += left.prefCount[i];
        }
        
        for (int i = 0; i < K; i++) {
            int newMod = (left.totalProd * i) % K;
            res.prefCount[newMod] += right.prefCount[i];
        }
        
        return res;
    }
    
    private void build(int node, int start, int end, int[] nums) {
        if (start == end) {
            tree[node] = new Node(K);
            tree[node].totalProd = nums[start] % K;
            tree[node].prefCount[nums[start] % K] = 1;
            return;
        }
        
        int mid = start + (end - start) / 2;
        build(2 * node, start, mid, nums);
        build(2 * node + 1, mid + 1, end, nums);
        
        tree[node] = merge(tree[2 * node], tree[2 * node + 1]);
    }
    
    private void update(int node, int start, int end, int idx, int val) {
        if (start == end) {
            tree[node] = new Node(K);
            tree[node].totalProd = val % K;
            tree[node].prefCount[val % K] = 1;
            return;
        }
        
        int mid = start + (end - start) / 2;
        if (idx <= mid) {
            update(2 * node, start, mid, idx, val);
        } else {
            update(2 * node + 1, mid + 1, end, idx, val);
        }
        
        tree[node] = merge(tree[2 * node], tree[2 * node + 1]);
    }
    
    private Node query(int node, int start, int end, int l, int r) {
        if (r < start || end < l) return null;
        if (l <= start && end <= r) return tree[node];
        
        int mid = start + (end - start) / 2;
        Node left = query(2 * node, start, mid, l, r);
        Node right = query(2 * node + 1, mid + 1, end, l, r);
        
        return merge(left, right);
    }
    
    public int[] resultArray(int[] nums, int k, int[][] queries) {
        int n = nums.length;
        this.K = k;
        this.tree = new Node[4 * n + 1];
        
        build(1, 0, n - 1, nums);
        
        int[] ans = new int[queries.length];
        
        for (int i = 0; i < queries.length; i++) {
            int index = queries[i][0];
            int value = queries[i][1];
            int startIdx = queries[i][2];
            int x = queries[i][3];
            
            update(1, 0, n - 1, index, value);
            
            Node res = query(1, 0, n - 1, startIdx, n - 1);
            
            if (res != null) {
                ans[i] = res.prefCount[x];
            } else {
                ans[i] = 0;
            }
        }
        
        return ans;
    }
}